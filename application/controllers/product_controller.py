...
def create_product(data: ProductItem):
    try:
...
def get_product(product_id: str):
    try:
        ...
        raise JSONResponse(content={"message": "Error fetching product", "error": str(e)}, status_code=500)

@product_router.put("/products/{product_id}")
def update_product(product_id: str, update_data: ProductItem):
    try:
...
def create_category(data: CategoryItem):
    try:
...
def get_category(category_id: str):
    try:
        ...
        raise JSONResponse(content={"message": "Error fetching category", "error": str(e)}, status_code=500)

@product_router.put("/categories/{category_id}")
def update_category(category_id: str, update_data: UpdateCategoryItem):
    try:
        updated_category = product_service.update_category(category_id, update_data)
...
def deactivate_category(category_id: str, update_data: DeactivateCategoryItem):
    try:
        deactivated_category = product_service.deactivate_category(category_id, update_data)
...
def create_measure_unit(data: MeasureUnitItem):
    try:
        measure_unit = product_service.create_measure_unit(data)
...
def get_measure_unit(unit_id: str):
    try:
        measure_unit = product_service.get_measure_unit(unit_id)
        return measure_unit
    except Exception as e:
        raise JSONResponse(content={"message": "Error fetching measure unit", "error": str(e)}, status_code=500)

@product_router.put("/measure-units/{unit_id}")
def update_measure_unit(unit_id: str, update_data: UpdateMeasureUnitItem):
    try:
        updated_measure_unit = product_service.update_measure_unit(unit_id, update_data)
...
def deactivate_measure_unit(unit_id: str, update_data: DeactivateMeasureUnitItem):
    try:
        deactivated_unit = product_service.deactivate_measure_unit(unit_id, update_data)
